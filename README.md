...
**Repo:** https://github.com/androidwithabhishek/Navigation-In-Multimodule-Application-
...
└── feature
    ...
    └── dashboard           # Dashboard feature module (self-contained)
```
...
sealed class MainGraph {
    ...
    @Serializable
    data object DashboardScreen : MainGraph()
}
```
...
class AuthNavigationFeatureApiImpl : AuthNavigationFeatureApi {
    ...
    override fun registerGraph(
        ...
        navGraphBuilder.navigation<SubGraph.Auth>(
            startDestination = MainGraph.AuthScreen
        ) {
            composable<MainGraph.AuthScreen> {
                AuthScreen(
                    ...
                    onGoDashboardClick = {
                        navHostController.navigate(MainGraph.DashboardScreen)
                    }
                )
            }
        }
...
```kotlin
// feature/dashboard/DashboardNavigation.kt
...
class DashboardNavigationFeatureApiImpl : DashboardNavigationFeatureApi {
    ...
    override fun registerGraph(
        ...
        navGraphBuilder.navigation<SubGraph.Dashboard>(
            startDestination = MainGraph.DashboardScreen
        ) {
            composable<MainGraph.DashboardScreen> {
                DashboardScreen(
                    ...
                    onGoAuthClick = {
                        navHostController.navigate(MainGraph.AuthScreen)
                    }
                )
            }
        }
...
The `dashboard` module has an identical Hilt module binding `DashboardNavigationFeatureApi`.
...
fun providesDefaultNavigator(
    ...
    DefaultNavigator(
        authNavigationFeatureApi,
        dashboardNavigationFeatureApi,
    )
```
...
fun MainNavigation(
    ...
    NavHost(
        ...
        defaultNavigator.authNavigationFeatureApi
            .registerGraph(navHostController, this)

        defaultNavigator.dashboardNavigationFeatureApi
            .registerGraph(navHostController, this)
    }
}
```
...
* **Decoupled features** – `auth` and `dashboard` have zero compile-time knowledge of each other.
* **Type-safe navigation** – routes are `@Serializable` sealed classes, not error-prone string literals.
* **Scales cleanly** – adding a new feature module means: add a `NavigationFeatureApi`, implement it, bind it with Hilt, and plug it into `DefaultNavigator`. No changes are needed inside other features.
...
* Dagger Hilt
* Gradle Multi-Module Architecture (`app`, `common`, `feature:auth`, `feature:dashboard`)
